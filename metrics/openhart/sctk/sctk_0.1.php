 ...
 */
$inputT = array();
$inputT['reference-url'] = array('name'=>'reference-url', 'type'=>'xsd:string');
$inputT['hypothesis-url'] = array('name'=>'hypothesis-url', 'type'=>'xsd:string');
 ...
 * @var $outputT
 */
$outputT['result-url'] = array('name'=>'result-url', 'type'=>'xsd:string');
$outputT['summary-url'] = array('name'=>'summary-url', 'type'=>'xsd:string');
//== STOP EDITABLE ZONE
...
function callback($input) {
	 ...
	 */
	$inputReference = $input['reference-url'];
	$inputHypothesis = $input['hypothesis-url'];
    ...
    $execString = 'wget '.$inputReference.' -q -O '.$referenceFile;
    //$execString = 'cp '.$inputReference.' '.$localdir.'/'.$inputReference;
    system($execString, $returnCode);
	if ($returnCode) {
	    return new soap_fault('SERVER', '', 'Execution Error', 'Return code = ' . $returnCode .' '. $execString);
	}

    //Getting hypothesis file

	$hypothesisFile = $localdir.'/'.array_pop(explode("/",$inputHypothesis));
    //$execString = 'cp '.$inputHypothesis.' '.$localdir.'/'.$inputHypothesis;
	$execString = 'wget '.$inputHypothesis.' -q -O '.$hypothesisFile;
	system($execString, $returnCode);
	if ($returnCode) {
	    return new soap_fault('SERVER', '', 'Execution Error', 'Return code = ' . $returnCode .' '. $execString);
	}
	...
	//$execString = 'java -Xmx2G -jar bin/meteor-1.4.jar '.$hypothesisFile.' '.$referenceFile.' -m \'exact stem synonym\' -l en > '.$localdir.'/output.txt';
    $execString = './bin/sclite -r '.$referenceFile.' -h '.$hypothesisFile.' -i sp -e utf-8 -D -o rsum \"sgml\" -n output -O '.$localdir.' 1>/dev/null 2>/dev/null';
    //== STOP EDITABLE ZONE
    ...
    $result=array('result-url' => 'http://localhost/'.substr($localdir,9).'/output.sgml','summary-url' => 'http://localhost/'.substr($localdir,9).'/output.raw');
